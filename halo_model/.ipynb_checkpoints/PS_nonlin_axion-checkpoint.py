...
from axionHMcode.cosmology.basic_cosmology import func_rho_comp_0
...
def func_full_halo_model_ax(M, power_spec_dic, cosmo_dic, hmcode_dic, axion_dic, 
                            ...
                            two_halo_damping = False, concentration_param=False, full_2h=True, full_output=False):
    """ 
    ...
    ############# Cold matter term ##########
    #########################################
    PS_cold_full = func_non_lin_PS_matter(M, k, PS_cold, cosmo_dic, hmcode_dic, cosmo_dic['Omega_db_0'], 
                                            alpha = alpha, eta_given = eta_given, ax_one_halo=False, one_halo_damping = one_halo_damping, 
                                            two_halo_damping = two_halo_damping, concentration_param=concentration_param, full_2h=full_2h, axion_dic=axion_dic)
    PS_cold_nonlin = PS_cold_full[0].copy()
    ...
    if full_output:
        if full_2h:
            return PS_total_matter, PS_cold_full[0], PS_cold_full[1], PS_cold_full[2], PS_total_cross, PS_total_ax, c_min, PS_cold_full[3], PS_cold_full[4], PS_cold_full[5], halo_mass_func_arr, halo_mass_func_arr_2, dens_profile_cold_arr_2, dens_profile_ax_arr_2 
        else:
            return PS_total_matter, PS_cold_full[0], PS_cold_full[1], PS_cold_full[2], PS_total_cross, PS_total_ax, c_min, PS_cold_full[3], PS_cold_full[4], PS_cold_full[5], halo_mass_func_arr_2, halo_mass_func_arr_2, dens_profile_cold_arr_2, dens_profile_ax_arr_2
            # note the duplicate output here
