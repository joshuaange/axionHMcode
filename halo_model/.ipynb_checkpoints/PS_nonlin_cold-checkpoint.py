...
#own pachages
from axionHMcode.cosmology.basic_cosmology import *
...
def func_non_lin_PS_matter(M, k, PS, cosmo_dic, hmcode_dic, Omega_0, 
                           ...
                           axion_dic=None,
                           custom_NFW_suppression=None,
                           custom_NFW_FT=None, full_output=False, input_squared_NFW_FT=False):
    """ 
    ...
    if custom_NFW_suppression is not None:
        dens_profile_arr *=  custom_NFW_suppression[None, :]
        # custom_NFW_suppression takes an array of k that multiplies the Fourier transform of the NFW profile
    if custom_NFW_FT is not None:
        dens_profile_arr = custom_NFW_FT
    halo_mass_func_arr = func_halo_mass_function(M, k, PS, cosmo_dic, Omega_0)
    f_ax = cosmo_dic['Omega_ax_0']/cosmo_dic['Omega_m_0']

    if input_squared_NFW_FT and full_2h:
        print("Not compatible settings")
    if input_squared_NFW_FT:
        integrand_arr_one = M[:, None]**2 * halo_mass_func_arr[:, None] * dens_profile_arr
    else:
    ...
    if full_output:
        return (one_halo**(alphacdm) + two_halo**(alphacdm))**(1/alphacdm), one_halo, two_halo, halo_mass_func_arr, M, dens_profile_arr, c_min
